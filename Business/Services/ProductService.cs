...
public class ProductService(IProductRepository productRepository) : IProductService
{
    ...
    public async Task<Product> CreateProductAsync(ProductRegistrationForm form)
    {
        ...
        var entity = await _productRepository.GetProductEntityAsync(x => x.ProductName == form.ProductName);
        entity ??= await _productRepository.CreateProductEntityAsync(ProductFactory.Create(form));
    ...
    {
        var entities = await _productRepository.GetAllProductEntityAsync();
    ...
    {
        var entity = await _productRepository.GetProductEntityAsync(expression);
    ...
    {
        var entity = await _productRepository.UpdateProductEntityAsync(ProductFactory.Create(form));
        var product = ProductFactory.Create(entity);
        return product ?? null!;
    }
    ...
    {
        var result = await _productRepository.DeleteProductEntityAsync(x => x.Id == id);
        return result;
    }
